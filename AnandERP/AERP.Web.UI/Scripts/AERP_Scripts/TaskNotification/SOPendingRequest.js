...
var SOPendingRequest = {
    ...
    Initialize: function () {
        ...
        SOPendingRequest.constructor();
        //SOPendingRequest.initializeValidation();
    },
    //Attach all event of page
    constructor: function () {
        // Create new record

        $('#CreateApprovedSalesOrderMaster').on("click", function () {
            SOPendingRequest.ActionName = "Approved";
            SOPendingRequest.AjaxCallSOPendingRequest();
        });
    },
    ...
    AjaxCallSOPendingRequest: function () {
        var SOPendingRequestData = null;
        if (SOPendingRequest.ActionName == "Approved") {
            SOPendingRequestData = null;
            SOPendingRequestData = SOPendingRequest.GetSOPendingRequest();
            ajaxRequest.makeRequest("/SalesOrderMasterAndDetails/SalesOrderMasterApproval", "POST", SOPendingRequestData, SOPendingRequest.Success);
        }
    },
    //Get properties data from the Create, Update and Delete page
    GetSOPendingRequest: function () {
        var Data = {
        };
        
        if (SOPendingRequest.ActionName == "Approved" || SOPendingRequest.ActionName == "Rejected") {
            Data.ID = $('#ID').val();
    ...
    Success: function (data) {
        ...
        SOPendingRequest.ReloadList(splitData[0], splitData[1], splitData[2]);
    ...
    //            }
    //            else if (SOPendingRequest.ActionName == "Rejected") {
    ...
    //    SOPendingRequest.XMLstring = ParameterXml + "</rows>";
