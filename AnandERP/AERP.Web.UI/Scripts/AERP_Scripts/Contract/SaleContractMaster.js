    ...
    constructor: function () {
        ...
        $('#RenewSaleContractMasterRecord').unbind("click").on("click", function () {
            ...
            if (TaskCode == 'GeneralRenewContract') {
                ...
                else if ($("#BillingType").val() == "2" && ($("#FixedBillingType").val() == "1" || $("#FixedBillingType").val() == "4") && ($("#FixedBillingForManPowerItemID").val() == "" || $("#FixedBillingForManPowerItemID").val() == "0")) {
                    notify("Please Select Man Power Item For Fix Billing", "warning");
                    return false;
                }
            ...
            $.ajax(
                  {
                      ...
                      type: "GET",
                      dataType: "html",
                      data: { "TaskCode": TaskCode, "SaleContractMasterID": SaleContractMasterID, "ContractNumber": ContractNumber },
                      url: Newurl,
    ...
    SaveSuccess: function (data) {
        ...
        if (splitData[1] == 'success') {
            ...
            $.ajax({
                ...
                type: "GET",
                dataType: "json",
                data: { "SaleContractMasterID": data.ID },
                url: '/SaleContractMaster/CheckSaleContractApprovalStatus',
                success: function (result) {
                    if (result == 1) {
                        SaleContractMaster.LoadSaleContractDetailsHome(data.ID, data.ContractNumber);
                    } else {
                        if (TaskCode == "GeneralRenewContract") {
