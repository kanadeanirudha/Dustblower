...
var PFChallanRemittance = {
    ...
    constructor: function () {
        ...
        $("#btnShowDetails").unbind('click').click(function () {
            ...
            var ESICZoneID = $('#ESICZoneID').val();
            if ((MonthName > 0) && (MonthYear > 0) && (ESICZoneID > 0) && CentreCode != "")
            {
                $("#UploadFile").show(true);
                $("#UploadFilelink").attr("href", "PFChallanRemittance/DownloadTxtFile?MonthName=" + MonthName + "&MonthYear=" + MonthYear + "&CentreCode=" + CentreCode + "&ESICZoneID=" + ESICZoneID)


                PFChallanRemittance.LoadList(MonthName, MonthYear, CentreCode, ESICZoneID);
            ...
            else if (CentreCode == '' || CentreCode == 0) {
                notify('Please select Centre', 'warning');
                $("#UploadFile").hide(true);
                $("#tblDataPRQ tbody tr").remove();
                return false;
            }
            else if (ESICZoneID == '' || ESICZoneID == 0) {
                notify('Please select Zone', 'warning');
                $("#UploadFile").hide(true);
                $("#tblDataPRQ tbody tr").remove();
                return false;
            }
    ...
    LoadList: function (MonthName, MonthYear, CentreCode) {
        debugger;
        $.ajax(
         {
             cache: false,
             type: "POST",
             data: { MonthName: MonthName, MonthYear: MonthYear, CentreCode: CentreCode },
             dataType: "html",
    ...
    ReloadList: function (message, colorCode, actionMode) {
        ...
        $.ajax(
        {
            ...
            type: "POST",
            dataType: "html",
            data: { actionMode: actionMode, "MonthName": MonthName, "MonthYear": MonthYear, CentreCode: CentreCode },
            url: '/PFChallanRemittance/List',
            success: function (data) {
                //Rebind Grid Data
