...
namespace AERP.Web.UI.Controllers
{
    public class ESICZoneMasterController : BaseController
    {
        ...
        [HttpGet]
        public ActionResult CreateESICZoneMasterCity(Int16 ID)
        {
            ESICZoneMasterViewModel model = new ESICZoneMasterViewModel();
            model.ID = ID;
            ...
            List<GeneralCityMaster> generalCityMasterList = GetListGeneralCityMaster(Convert.ToString(0));
            ...
            return PartialView("/Views/Salary/ESICZoneMaster/CreateCity.cshtml", model);
        }

        [HttpPost]
        public ActionResult CreateESICZoneMasterCity(ESICZoneMasterViewModel model)
        {
            try
            {
                //if (ModelState.IsValid)
                ...
                {
                    model.ESICZoneMasterDTO.ConnectionString = _connectioString;
                    model.ESICZoneMasterDTO.CityID = model.CityID;
                    model.ESICZoneMasterDTO.ID = model.ID;
                    model.ESICZoneMasterDTO.CreatedBy = Convert.ToInt32(Session["UserID"]);
                    IBaseEntityResponse<ESICZoneMaster> response = _ESICZoneMasterBA.InsertESICZoneMasterCity(model.ESICZoneMasterDTO);
        ...
        public ActionResult DeleteCity(int ID)
        {
            ...
            if (ID > 0)
            {
                ...
                ESICZoneMaster ESICZoneMasterDTO = new ESICZoneMaster();
                ESICZoneMasterDTO.ConnectionString = _connectioString;
                ESICZoneMasterDTO.ESICZoneDetailsID = Convert.ToInt16(ID);
                ESICZoneMasterDTO.DeletedBy = Convert.ToInt32(Session["UserID"]);
                response = _ESICZoneMasterBA.DeleteESICZoneMasterCity(ESICZoneMasterDTO);
                errorMessage = CheckError((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Delete);
            }
        ...
        public IEnumerable<ESICZoneMasterViewModel> GetESICZoneMaster(out int TotalRecords)
        {
            ...
            if (Enum.TryParse(_actionMode, out actionModeEnum))     // checks actionMode i.e. Insert or Update // 
            {
                ...
                {
                    searchRequest.SortBy = "A.ZoneName";
        ...
        public ActionResult AjaxHandler(JQueryDataTableParamModel param)
        {
            if (Session["UserType"] != null)
            {
                ...
                var result = from c in records select new[] { Convert.ToString(c.ZoneName), Convert.ToString(c.ZoneCode), Convert.ToString(c.ID), Convert.ToString(c.IsDefault), Convert.ToString(c.CityName), Convert.ToString(c.ESICZoneDetailsID) };
