    ...
    public class SalesOrderMasterAndDetailsController : BaseController
    {
        ...
        public FileStreamResult Download(int id)
        {
            ...
            try
            {
                ...
                //SalesOrderPDF = SalesOrderPDF + "</table>";
                DownloadPDF1(SalesOrderPDF, model.SalesOrderNumber, model.SalesOrderList[0].CustomerMasterID, model.SalesOrderList[0].IsDeleted, model.SalesOrderList[0].WaterMark);
        ...
        public void DownloadPDF1(string SalesOrderPDF, string SalesOrderNumber, int CustomerMasterID, bool IsDeleted, string WaterMark)
        {
            ...
            Response.Cache.SetCacheability(HttpCacheability.NoCache);
            Response.BinaryWrite(GetPDF(SalesOrderPDF, IsDeleted, WaterMark));
            Response.End();
        }
        ...
        [HttpGet]
        public ActionResult SalesOrderMasterApproval(int PersonID, string TNDID, string TNMID, string GTRDID1, string TaskCode, string StageSequenceNumber, string IsLast)
        {
            SalesOrderMasterAndDetailsViewModel model = new SalesOrderMasterAndDetailsViewModel();
            try
            {
                ...
                model.SalesOrderList = GetSalesOrderRecord(PersonID, Convert.ToInt32(TNMID), model.GeneralTaskReportingDetailsID);
                model.ID = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].ID : 0;
                model.GeneralUnitsID = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].GeneralUnitsID : new int();
                model.PurchaseOrderNumberClient = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].PurchaseOrderNumberClient : null;
                model.CustomerName = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].CustomerName : null;
                model.CustomerBranchMasterName = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].CustomerBranchMasterName : null;
                model.ContactPersonName = model.SalesOrderList.Count > 0 ? model.SalesOrderList[0].ContactPersonName : null;
                ...
                ViewBag.GeneralUnitMasterList = new SelectList(GeneralUnitMasterList, "Value", "Text", model.GeneralUnitsID);

                return PartialView("/Views/Sales/SalesOrderMasterAndDetails/SalesOrderMasterApproval.cshtml", model);
            }
            catch (Exception ex)
            {
                _logException.Error(ex.Message);
        ...
        [HttpPost]
        public ActionResult SalesOrderMasterApproval(SalesOrderMasterAndDetailsViewModel model)
        {
            try
            {
                if (model != null && model.SalesOrderMasterAndDetailsDTO != null)
                {
                    ...
                    model.SalesOrderMasterAndDetailsDTO.ConnectionString = _connectioString;
                    model.SalesOrderMasterAndDetailsDTO.PersonID = model.PersonID;
                    model.SalesOrderMasterAndDetailsDTO.ID = model.ID;
                    model.SalesOrderMasterAndDetailsDTO.IsLastRecord = Convert.ToBoolean(model.IsLastRecord);
                    model.SalesOrderMasterAndDetailsDTO.TaskNotificationMasterID = model.TaskNotificationMasterID;
                    model.SalesOrderMasterAndDetailsDTO.TaskNotificationDetailsID = model.TaskNotificationDetailsID;
                    model.SalesOrderMasterAndDetailsDTO.GeneralTaskReportingDetailsID = model.GeneralTaskReportingDetailsID;
                    model.SalesOrderMasterAndDetailsDTO.StageSequenceNumber = model.StageSequenceNumber;
                    model.SalesOrderMasterAndDetailsDTO.ApprovedStatus = model.ApprovedStatus;
                    model.SalesOrderMasterAndDetailsDTO.CreatedBy = Convert.ToInt32(Session["UserID"]);

                    IBaseEntityResponse<SalesOrderMasterAndDetails> response = _SalesOrderMasterAndDetailsBA.InsertSalesOrderApproval(model.SalesOrderMasterAndDetailsDTO);
                    model.SalesOrderMasterAndDetailsDTO.errorMessage = CheckError((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Insert);
                }
                return Json(model.SalesOrderMasterAndDetailsDTO.errorMessage, JsonRequestBehavior.AllowGet);
        ...
        [NonAction]
        protected List<SalesOrderMasterAndDetails> GetSalesOrderRecord(int personId, int taskNotificationMasterID, int GeneralTaskReportingDetailsID)
        {
            SalesOrderMasterAndDetailsSearchRequest searchRequest = new SalesOrderMasterAndDetailsSearchRequest();
            ...
            List<SalesOrderMasterAndDetails> listSalesOrderDetails = new List<SalesOrderMasterAndDetails>();
            IBaseEntityCollectionResponse<SalesOrderMasterAndDetails> baseEntityCollectionResponse = _SalesOrderMasterAndDetailsBA.GetSalesOrderForApproval(searchRequest);
            if (baseEntityCollectionResponse != null)
            {
                if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                {
                    listSalesOrderDetails = baseEntityCollectionResponse.CollectionResponse.ToList();
                }
            }
