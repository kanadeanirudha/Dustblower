    ...
    public class SalesOrderDeliveryMasterAndDetailsController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            if (Convert.ToString(Session["UserType"]) == "A" || ((Convert.ToInt32(Session["Sales Manager"]) > 0 || Convert.ToInt32(Session["Sales Manager:Entity"]) > 0 || Convert.ToInt32(Session["Store Manager"]) > 0) && IsApplied == true))
            {
                ...
                List<SelectListItem> DMStatus = new List<SelectListItem>();
                ViewBag.DMStatus = new SelectList(DMStatus, "Value", "Text");
                List<SelectListItem> DMStatus_li = new List<SelectListItem>();
                DMStatus_li.Add(new SelectListItem { Text = "All", Value = "0" });
                DMStatus_li.Add(new SelectListItem { Text = "Cancelled", Value = "1" });
                DMStatus_li.Add(new SelectListItem { Text = "Pending Invoice", Value = "2" });

                ViewData["DMStatus"] = new SelectList(DMStatus_li, "Value", "Text");
        ...
        public ActionResult Create(SalesOrderDeliveryMasterAndDetailsViewModel model)
        {
            try
            {
                ...
                if (model != null && model.SalesOrderDeliveryMasterAndDetailsDTO != null)
                {
                    ...
                    model.SalesOrderDeliveryMasterAndDetailsDTO.PurchaseAmount = model.PurchaseAmount;
                    model.SalesOrderDeliveryMasterAndDetailsDTO.PurchaseBillAmount = model.PurchaseBillAmount;
        ...
        [HttpGet]
        public ActionResult ViewDetailsSaleOrderDelivery(int id)
        {
            try
            {
                SalesOrderDeliveryMasterAndDetailsViewModel model = new SalesOrderDeliveryMasterAndDetailsViewModel();
                model.SalesOrderDeliveryMasterAndDetailsDTO.SalesOrderDeliveryMasterID = id;
                model.SalesOrderDeliveryMasterAndDetailsList = GetSaleOrderDeliveryMemoDetailsByID(id);
                //model.PurchaseRequisitionMasterList = GetPurchaseRequisitionMasterList(id);
                if (model.SalesOrderDeliveryMasterAndDetailsList.Count > 0 && model.SalesOrderDeliveryMasterAndDetailsList != null)
                {
                    model.DeliveryNumber = model.SalesOrderDeliveryMasterAndDetailsList[0].DeliveryNumber;
                ...
                return PartialView("/Views/Sales/SalesOrderDeliveryMasterAndDetails/ViewDetailsSaleOrderDelivery.cshtml", model);
            }
            catch (Exception ex)
            {
                _logException.Error(ex.Message);
        ...
        public FileStreamResult Download(SalesOrderDeliveryMasterAndDetailsViewModel _model)
        {
            ...
            try
            {
                ...
                //SalesOrderPDF = SalesOrderPDF + "</table>";
                DownloadPDF1(SalesOrderPDF, model.DeliveryNumber, model.SalesOrderDMList[0].CustomerMasterID, model.SalesOrderDMList[0].IsDeleted, _model.WithDMRate, model.SalesOrderDMList[0].WaterMark);
        ...
        public FileStreamResult DownloadAnnexure(SalesOrderDeliveryMasterAndDetailsViewModel _model)
        {
            ...
            try
            {
                ...
                SalesOrderPDF = SalesOrderPDF + "<table style='width:38%' ><tr><th style='font-size:8pt;text-align:left'>Authorised Signatory</th></tr><tr><td style='text-align:left'> ___________________</td></tr></table></td></tr></table>";
                //SalesOrderPDF = SalesOrderPDF + "</table>";
                DownloadPDFAnn(SalesOrderPDF, model.DeliveryNumber, model.SalesOrderDMList[0].CustomerMasterID, model.SalesOrderDMList[0].IsDeleted, model.SalesOrderDMList[0].WaterMark);
                MemoryStream workStream = new MemoryStream();
                return new FileStreamResult(workStream, "application/pdf");
        ...
        public void DownloadPDF1(string SalesOrderPDF, string DeliveryNumber, int CustomerMasterID, bool IsDeleted, byte WithDMRate, string WaterMark)
        {
        ...
        public void DownloadPDFAnn(string SalesOrderPDF, string DeliveryNumber, int CustomerMasterID, bool IsDeleted, string WaterMark)
        {
            //string HTMLContent = "Hello <b>World</b>";
        ...
        // Non-Action Method
        #region Methods
        public IEnumerable<SalesOrderDeliveryMasterAndDetailsViewModel> GetSalesOrderDeliveryMasterAndDetails(out int TotalRecords, Byte SOStatus, int CustomerMasterID, int CustomerBranchMasterID, int AdminRoleID, string MonthName, string MonthYear, Byte DMStatus)
        {
            ...
            searchRequest.DMStatus = DMStatus;
        ...
        [NonAction]
        protected List<SalesOrderDeliveryMasterAndDetails> GetSaleOrderDeliveryMemoDetailsByID(int id)
        {
            SalesOrderDeliveryMasterAndDetailsSearchRequest searchRequest = new SalesOrderDeliveryMasterAndDetailsSearchRequest();
            searchRequest.ConnectionString = Convert.ToString(ConfigurationManager.ConnectionStrings["Main.ConnectionString"]);
            searchRequest.ID = id;
            List<SalesOrderDeliveryMasterAndDetails> listPurchaseGRNMaster = new List<SalesOrderDeliveryMasterAndDetails>();
            IBaseEntityCollectionResponse<SalesOrderDeliveryMasterAndDetails> baseEntityCollectionResponse = _SalesOrderDeliveryMasterAndDetailsBA.GetDeliveryMemoSaleOrderDetailsByID(searchRequest);
            if (baseEntityCollectionResponse != null)
            {
                if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                {
        ...
        // AjaxHandler Method
        #region AjaxHandler
        public ActionResult AjaxHandler(JQueryDataTableParamModel param, Byte SOStatus, int CustomerMasterID, int CustomerBranchMasterID, int AdminRoleID, string MonthName, string MonthYear, byte DMStatus)
        {
            if (Session["UserType"] != null)
            {
                ...
                filteredGroupDescription = GetSalesOrderDeliveryMasterAndDetails(out TotalRecords, SOStatus, CustomerMasterID, CustomerBranchMasterID, AdminRoleID, MonthName, MonthYear, DMStatus);


                var records = filteredGroupDescription.Skip(0).Take(param.iDisplayLength);
                var result = from c in records select new[] { Convert.ToString(c.SalesOrderNumber), Convert.ToString(c.IsLocked), Convert.ToString(c.SalesOrderMasterID), Convert.ToString(c.DeliveryTransDate), Convert.ToString(c.SalesOrderMasterID), Convert.ToString(c.DeliveryNumber), Convert.ToString(c.SalesOrderDeliveryMasterID), Convert.ToString(c.IsInvoiced), Convert.ToString(c.IsCancelled) };
