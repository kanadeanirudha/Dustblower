...
namespace AERP.DataProvider
{
    public class InventoryReportDataProvider : DBInteractionBase, IInventoryReportDataProvider
    {
        ...
        public IBaseEntityCollectionResponse<InventoryReport> GetSaleOrderMonthlyDeliveryReportReportList(InventoryReportSearchRequest searchRequest)
        {
            //throw new NotImplementedException();
            IBaseEntityCollectionResponse<InventoryReport> baseEntityCollection = new BaseEntityCollectionResponse<InventoryReport>();
            SqlCommand cmdToExecute = new SqlCommand();
            ...
            try
            {
                ...
                else
                {
                    ...
                    cmdToExecute.Connection = _mainConnection;
                    cmdToExecute.CommandText = "dbo.USP_SaleOrderDeliveryReport";
                    cmdToExecute.CommandType = CommandType.StoredProcedure;
                    //-----------------------------------------------------Output Parameters ------------------------------------------------------------------
                    if (searchRequest.SalaryMonth != null)
                    {
                        cmdToExecute.Parameters.Add(new SqlParameter("@tiMonth", SqlDbType.TinyInt, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, Convert.ToInt16(searchRequest.SalaryMonth)));
                    }
                    else
                    {
                        cmdToExecute.Parameters.Add(new SqlParameter("@tiMonth", SqlDbType.TinyInt, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, DBNull.Value));
                    }
                    if (searchRequest.SalaryYear != "")
                    {
                        cmdToExecute.Parameters.Add(new SqlParameter("@tiYear", SqlDbType.Int, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, Convert.ToInt32(searchRequest.SalaryYear)));
                    }
                    else
                    {
                        cmdToExecute.Parameters.Add(new SqlParameter("@tiYear", SqlDbType.Int, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, DBNull.Value));
                    }
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        item.CostDiff = sqlDataReader["CostDiff"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["CostDiff"]);
                        item.SaleCost = sqlDataReader["SaleCost"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["SaleCost"]);
                        item.PurchaseCost = sqlDataReader["PurchaseCost"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["PurchaseCost"]);
                        item.DeliveryNumber = sqlDataReader["DeliveryNumber"] is DBNull ? string.Empty : Convert.ToString(sqlDataReader["DeliveryNumber"]);
                        item.DeliveryTransDate = sqlDataReader["DeliveryTransDate"] is DBNull ? DateTime.Now : Convert.ToDateTime(sqlDataReader["DeliveryTransDate"]);
                        item.ProfitLossPercentage = sqlDataReader["ProfitLossPercentage"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["ProfitLossPercentage"]);
                        item.IsProfit = sqlDataReader["IsProfit"] is DBNull ? new bool() : Convert.ToBoolean(sqlDataReader["IsProfit"]);
                        item.SaleCostWTGST = sqlDataReader["SaleCostWTGST"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["SaleCostWTGST"]);

                        item.PurchaseCosWTGST = sqlDataReader["PurchaseCosWTGST"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["PurchaseCosWTGST"]);
                        item.ProfitLossPercentage = sqlDataReader["ProfitLossPercentage"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["ProfitLossPercentage"]);
        ...
        public IBaseEntityCollectionResponse<InventoryReport> GetInventoryStockRegisterMonthWiseReportList(InventoryReportSearchRequest searchRequest)
        {
            //throw new NotImplementedException();
            IBaseEntityCollectionResponse<InventoryReport> baseEntityCollection = new BaseEntityCollectionResponse<InventoryReport>();
            SqlCommand cmdToExecute = new SqlCommand();
            ...
            try
            {
                ...
                else
                {
                    ...
                    cmdToExecute.Connection = _mainConnection;
                    cmdToExecute.CommandText = "dbo.USP_InventoryStockRegisterMonthWise";
                    cmdToExecute.CommandType = CommandType.StoredProcedure;
                    cmdToExecute.CommandTimeout = 0;
                    //-----------------------------------------------------Output Parameters ------------------------------------------------------------------

                    cmdToExecute.Parameters.Add(new SqlParameter("@iMonth", SqlDbType.TinyInt, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, searchRequest.SalaryMonth));
                    cmdToExecute.Parameters.Add(new SqlParameter("@iYear", SqlDbType.Int, 4, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, searchRequest.SalaryYear));
                    cmdToExecute.Parameters.Add(new SqlParameter("@nsCentreCode", SqlDbType.NVarChar, 35, ParameterDirection.Input, true, 10, 0, "", DataRowVersion.Proposed, searchRequest.CentreCode));
                    cmdToExecute.Parameters.Add(new SqlParameter("@iErrorCode", SqlDbType.Int, 4, ParameterDirection.Output, true, 10, 0, "", DataRowVersion.Proposed, _errorCode));
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        item.ItemNumber = sqlDataReader["ItemNumber"] is DBNull ? string.Empty : Convert.ToString(sqlDataReader["ItemNumber"]);
                        item.ItemDescription = sqlDataReader["ItemDescription"] is DBNull ? string.Empty : Convert.ToString(sqlDataReader["ItemDescription"]);
                        item.Opening = sqlDataReader["Opening"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["Opening"]);
                        item.Inward = sqlDataReader["Inward"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["Inward"]);
                        item.Outward = sqlDataReader["Outward"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["Outward"]);
                        item.Closing = sqlDataReader["Closing"] is DBNull ? new decimal() : Convert.ToDecimal(sqlDataReader["Closing"]);
                        item.CentreName = searchRequest.CentreName;
                        item.MonthReport = searchRequest.SalaryMonth;
                        item.MonthName = searchRequest.MonthName;
                        item.YearReport = searchRequest.SalaryYear;
                        baseEntityCollection.CollectionResponse.Add(item);
                    }
